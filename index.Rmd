...
>
> The data were pulled from the USGS. The gage station site id is `01049500` and is located in [Gardiner , Maine (USA)](https://waterdata.usgs.gov/monitoring-location/01049500/#parameterCode=00065&period=P7D). The data span the years 1976 through 2021.
>
...
In both cases, the patterns appear similar with a relatively linear discharge rate between the years 1975 and 1998 and a parabolic shape between 1998 and 2021.
...
ggplot(dat, aes(Date, res1)) + geom_point(alpha = 0.1, cex = 0.5) + 
  ...
  geom_smooth(method = "lm", se = FALSE,
              method.args = list(family = "symmetric")) +
  scale_x_continuous(breaks = c(as.Date("1976-1-1"), as.Date("2021-1-1")), label = c("1977", "2021")) +
